...
@section('content')
        ...
        <div class="container">
            <div class="row no-gutters slider-text align-items-center justify-content-center">
                <div class="col-md-9 ftco-animate text-center">
                    <p class="breadcrumbs"><span class="mr-2"><a href="index.html">Trang chủ</a></span> <span>Các loại mặt hàng</span>
                    </p>
    ...
    <section class="ftco-section">
                ...
                <div class="col-md-10 mb-5 text-center">
                    <ul class="product-category">
                        @foreach($menu_brand as $brand)
                            <li style="margin: 0px 20px 0px 20px;border: 1px solid #eeeeee;"><a
                                    href="{{route('shop.brand', ['slug'=> $brand->slug]) }}"><img
                                        src="{{ asset($brand->image) }}" alt="" width="80px"></a></li>
                        @endforeach
                        ...
                        <li class="nav-item dropdown">
                            <a class="nav-link dropdown-toggle" data-toggle="dropdown" href="#" role="button"
                               aria-haspopup="true" aria-expanded="false">Sắp xếp</a>
        ...
        <div class="container">
            <div class="row">
                <div id="left-content" class="col-sm-3">
                    <div id="title" class="col-sm-12" style="padding: 0px">
                        <h3><a data-toggle="collapse" data-target="#demo" style="color: black">Danh Mục
                    ...
                    <div class="col-sm-12">
                        <ul id="category" class="" style="padding:0px">
                            ...
                            {{--@foreach($list as $item)
                                <li class="treeview">
                                    <a href="--}}{{--{{ route('shop.category', ['slug' => $item->slug]) }}--}}{{--"></i> {{ $item['category']->name }}</a>
                                    @foreach($listCat as $child)
                                        --}}{{-- @if($child->parent_id == $item->id)--}}{{--
                                        @if($child['category']->parent_id == $item['category']->id)
                                            <ul id="demo" class="collapse">
                                                <li><a href="--}}{{--{{ route('shop.category', ['slug' => $child->slug]) }}--}}{{--" style="color: black">{{$child['category']->name}}</a></li>
                                            </ul>
                                        @endif
                                    @endforeach
                                </li>
                ...
                <div class="col-md-9" style="display: flex; flex-wrap: wrap">
                        ...
                        <div class="col-md-6 col-lg-3 ftco-animate">
                            <div class="product">
                                <a href="{{ route('shop.productDetails',['slug' => $product->slug]) }}"
                                   class="img-prod"><img class="img-fluid" src="{{ asset($product->image)}}"
                                                         alt="Colorlib Template">
                                    <div class="overlay"></div>
                                </a>
                                <div class="text py-3 pb-4 px-3 text-center">
                                    <h3>
                                        <a href="{{ route('shop.productDetails',['slug' => $product->slug]) }}">{{ $product->name }}</a>
                                    </h3>
                                    <div class="d-flex">
                                        <div class="pricing">
                                            <p class="price"><span class="mr-2 price-dc">{{ number_format($product->price) }} đ</span><span>{{ number_format($product->sale) }} đ</span>
                                            </p>
                                        </div>
                                    </div>
                    ...
                    @endforeach
                        ...
                        @foreach($products_by_cat_child as $product)
                            <div class="col-md-6 col-lg-3 ftco-animate">
                                <div class="product">
                                    <a href="{{ route('shop.productDetails',['slug' => $product->slug]) }}"
                                       class="img-prod"><img class="img-fluid" src="{{ asset($product->image)}}"
            ...
            {{ $products_by_cat->links() }}
            {{ $products_by_cat_child->links() }}
        </div>
    </section>
